...
The last line is the provenance line (dEitY719/gh-issue-skills#43): it sits
**outside** `<details>` so it stays visible while the metrics block is
collapsed, and `<harness>` / `<model>` are whatever the writing agent knows
itself to be — `none` when it doesn't, never a guess.
...
bash "$PLUGIN_ROOT/lib/ai-metrics-footer.sh" \
    "$TOKENS" "$HUMAN_H" "$ELAPSED" "$SKILL" "$HARNESS" "$LLM_MODEL" >> "$BODY" \
...
The provenance line is part of the same footer unit, so the strip takes it
with the block — otherwise a re-append stacks a second copy
(dEitY719/gh-issue-skills#43 F-4). `gh-setup:add-ai-metrics` owns the mirror of
this regex (gh-setup-skills#16); change them together.
