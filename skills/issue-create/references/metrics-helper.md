...
`<harness>` / `<model>` are whatever the writing agent knows itself to be —
`none` when it doesn't, never a guess (dEitY719/gh-issue-skills#43).
...
`gh-setup:add-ai-metrics` owns the mirror of this regex (gh-setup-skills#16);
change them together, or a re-append stacks a second provenance line
(dEitY719/gh-issue-skills#43 F-4).
