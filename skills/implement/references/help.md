...
3. Checks the issue's origin line (`Generated by Harness(x) and LLM(y)`) before any write: a trusted harness starts immediately, anything else — including no line at all — has to pass a plausibility review first, and a failed review exits 2 having changed nothing (see `references/origin-trust.md`).
4. Claims the issue via `GH_HOST=<host> gh issue edit <N> --repo <owner>/<repo> --add-assignee @me` so teammates see it's being worked (soft-fail on error; see `references/claim.md`).
5. Depending on mode:
...
6. Auto-detects the test runner from AGENTS.md → `tox.ini` → `pyproject.toml` → `package.json` → `tests/*.bats`, using the first that matches, then records a pre-edit baseline of already-failing tests.
7. Pre-existing failures are reported separately, never fixed — on both paths. The 3-attempt test-failure loop applies to the fallback path only; the TDD path stops on judgment instead (same failure repeating, fix breaking a green test, unexplained failure).
8. Prints a compact report: changed files, which path ran (`tdd`/`fallback`), test result, next-step hint.
