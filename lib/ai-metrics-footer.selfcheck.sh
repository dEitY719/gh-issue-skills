...
# 9. Provenance line (dEitY719/gh-issue-skills#43): last line, outside <details>,
#    so it stays visible while the metrics block is collapsed. Three downstream
#    parsers read it (gh-flow-skills#36, #44, gh-setup-skills#16) — it is a wire
#    format, not decoration.
got=$(bash "$TARGET" 2000 4 3 gh-issue-create claude opus | tail -n 1)
chk "origin line carries harness and model" "$got" "Generated by Harness(claude) and LLM(opus)"
...
# 9c. It sits after `</details>`, separated by one blank line.
got=$(bash "$TARGET" 2000 4 3 gh-issue-create claude opus | sed -n '10p;11p;12p')
chk "origin line follows </details>" "$got" "</details>

Generated by Harness(claude) and LLM(opus)"

# 9d. Same refusal as the marker: a newline or `-->` in either value would
#     corrupt the block the parsers read.
...
# 9e. The opt-out silences the origin line too (NF-1) — one early exit covers it.
got=$(GH_DISABLE_AI_METRICS=1 bash "$TARGET" 2000 4 3 gh-issue-create claude opus; printf '|%s' "$?")
chk "GH_DISABLE_AI_METRICS silences the origin line" "$got" "|0"
